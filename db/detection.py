from base import BASE  # Ensure db/base.py exists and is in your PATH or use the absolute import
import numpy as np
...
class DETECTION(BASE):
    def __init__(self, db_config):
        ...
        # Configuration parameters for detection system
        ...
        # Gaussian parameters for bounding box detection
        self._configs["gaussian_bump"]   = True
        self._configs["gaussian_iou"]    = 0.7
        self._configs["gaussian_radius"] = -1

        # Randomization options for image augmentation
        self._configs["rand_color"] = False
        self._configs["rand_crop"]  = False

        # Update the configuration with database-specific configurations
        self.update_config(db_config)

        # Ensure that rand_scales are correctly initialized
